...
export class TypedEventEmitter<T extends { readonly [P in keyof T]: any[] }> {
  ...
  protected emit<P extends Exclude<keyof T, number>>(
    eventName: P,
    ...args: T[P]
  ): boolean {
  ...
  on<P extends Exclude<keyof T, number>>(
    eventName: P,
    listener: (...args: T[P]) => void
  ): this {
    this.#emitter.on(eventName, listener as any)
    return this
  }

  once<P extends Exclude<keyof T, number>>(
    eventName: P,
    listener: (...args: T[P]) => void
  ): this {
  ...
  off<P extends Exclude<keyof T, number>>(
    eventName: P,
    listener: (...args: T[P]) => void
  ): this {
