param(
  ...
  [string]$ServiceName = "attack-workbench",
...
$deploymentRoot = (Resolve-Path $DeploymentRepoPath).Path
...
Require-Path $composeFile "Could not find Attack Workbench deployment compose.yaml under '$DeploymentRepoPath'"
...
$deploymentStage = Join-Path $packageRoot "deployment"
$archiveBase = Join-Path $serviceRoot "$ServiceName.7z"

Write-Host "Cleaning work folders..."
Remove-Item $workRoot -Recurse -Force -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Force $deploymentStage | Out-Null

Write-Host "Copying upstream deployment bundle..."
Get-ChildItem $deploymentRoot -Force | Where-Object { $_.Name -notin @(".git", ".github", ".env", "workspace-data") } | ForEach-Object {
  Copy-Item $_.FullName $deploymentStage -Recurse -Force
}
