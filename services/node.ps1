Param(
  ...
  [string]$NODE_PROGRAM_PATH = ( Join-Path "${PWD}" "${SERVICE_NAME}" "node-${NODE_VERSION}-${NODE_OS}-${CPU_ARCH}" "bin" "node"),
  [string]$NPM_PROGRAM_PATH = ( Join-Path "${PWD}" "${SERVICE_NAME}" "node-${NODE_VERSION}-${NODE_OS}-${CPU_ARCH}" "bin" "npm"),
